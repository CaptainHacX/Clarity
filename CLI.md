...
Clarity can run entirely from the command line — no GUI window is opened. This is useful for scripting, IT admin workflows, and scheduled tasks beyond the built-in scheduler.

## Usage

```
clarity --cli [options] [categories...]
```
...
```bash
clarity --cli programs list --json | jq '.count'   # stdout is pure JSON
clarity --cli programs list --json 2>/dev/null     # discard progress entirely
```
...
```bash
clarity --cli metrics
clarity --cli metrics --json    # JSON array of metric objects
```

Start a persistent HTTP metrics server:

```bash
clarity --cli metrics-server              # default port 9100
clarity --cli metrics-server --port 9200  # custom port
