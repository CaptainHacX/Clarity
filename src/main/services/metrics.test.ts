...
// Mock electron app
vi.mock('electron', () => ({
  app: { getVersion: () => '1.17.0', getPath: () => '/tmp/clarity' }
}))
...
describe('collectMetrics', () => {
  ...
  it('includes clarity_info metric', async () => {
    const metrics = await collectMetrics()
    const info = metrics.find(m => m.name === 'clarity_info')
  ...
  it('includes system uptime metric', async () => {
    const metrics = await collectMetrics()
    const uptime = metrics.find(m => m.name === 'clarity_system_uptime_seconds')
  ...
  it('includes CPU usage metric', async () => {
    const metrics = await collectMetrics()
    const cpu = metrics.find(m => m.name === 'clarity_system_cpu_usage_percent')
  ...
  it('includes memory metrics', async () => {
    const metrics = await collectMetrics()
    const total = metrics.find(m => m.name === 'clarity_system_memory_total_bytes')
    const used = metrics.find(m => m.name === 'clarity_system_memory_used_bytes')
  ...
  it('includes history-based counters', async () => {
    const metrics = await collectMetrics()
    const scans = metrics.find(m => m.name === 'clarity_scans_total')
    const cleaned = metrics.find(m => m.name === 'clarity_items_cleaned_total')
    const space = metrics.find(m => m.name === 'clarity_space_saved_bytes_total')
    const errors = metrics.find(m => m.name === 'clarity_scan_errors_total')
  ...
  it('includes last scan metrics', async () => {
    const metrics = await collectMetrics()
    const ts = metrics.find(m => m.name === 'clarity_last_scan_timestamp_seconds')
    const dur = metrics.find(m => m.name === 'clarity_last_scan_duration_seconds')
    const items = metrics.find(m => m.name === 'clarity_last_scan_items_found')
...
describe('formatPrometheus', () => {
  it('formats a simple gauge metric', () => {
    const output = formatPrometheus([
      { name: 'clarity_test', type: 'gauge', help: 'A test metric', value: 42 },
    ])
    expect(output).toContain('# HELP clarity_test A test metric')
    expect(output).toContain('# TYPE clarity_test gauge')
    expect(output).toContain('clarity_test 42')
  })

  it('formats a gauge metric with large value', () => {
    const output = formatPrometheus([
      { name: 'clarity_count', type: 'gauge', help: 'A gauge', value: 100 },
    ])
    expect(output).toContain('# TYPE clarity_count gauge')
    expect(output).toContain('clarity_count 100')
  })

  it('formats labels correctly', () => {
    const output = formatPrometheus([
      { name: 'clarity_info', type: 'gauge', help: 'Info', labels: { version: '1.17.0', platform: 'win32' }, value: 1 },
    ])
    expect(output).toContain('clarity_info{version="1.17.0",platform="win32"} 1')
  })

  it('escapes label values', () => {
    const output = formatPrometheus([
      { name: 'clarity_test', type: 'gauge', help: 'Test', labels: { path: 'C:\\Users\\test' }, value: 1 },
    ])
  ...
  it('handles metrics without labels', () => {
    const output = formatPrometheus([
      { name: 'clarity_uptime', type: 'gauge', help: 'Uptime', value: 3600 },
    ])
    expect(output).toContain('clarity_uptime 3600')
