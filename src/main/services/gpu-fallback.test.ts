...
describe('gpu-fallback', () => {
  beforeEach(() => {
    h.dataDir.value = mkdtempSync(join(tmpdir(), 'clarity-gpu-'))
    h.switches.length = 0
    for (const k of Object.keys(h.listeners)) delete h.listeners[k]
    h.relaunch.mockClear()
    h.exit.mockClear()
    process.argv = ['node', 'clarity']
    delete process.env.CLARITY_DISABLE_GPU
  })
  ...
  it('honors the --disable-gpu flag', async () => {
    process.argv = ['node', 'clarity', '--disable-gpu']
    const { shouldDisableGpu } = await load()
    expect(shouldDisableGpu()).toBe(true)
  })

  it('honors the CLARITY_DISABLE_GPU env var', async () => {
    process.env.CLARITY_DISABLE_GPU = '1'
    const { shouldDisableGpu } = await load()
    expect(shouldDisableGpu()).toBe(true)
  })
