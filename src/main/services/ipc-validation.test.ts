...
describe('validateWifiExportPayload', () => {
  const validPayload = {
    exportedAt: 1738000000000,
    generatedBy: 'Clarity Wi-Fi Tool',
    connected: null,
    networks: [{ ssid: 'Home', channel: 6 }],
    samples: { Home: [{ t: 1, signalDbm: -60, noiseDbm: -91 }] },
  }

  it('accepts a well-formed payload', () => {
    expect(validateWifiExportPayload(validPayload)).toEqual(validPayload)
  })

  it('rejects non-objects and arrays', () => {
    expect(validateWifiExportPayload(null)).toBeNull()
    expect(validateWifiExportPayload('x')).toBeNull()
    expect(validateWifiExportPayload([])).toBeNull()
  })

  it('rejects missing/invalid fields', () => {
    expect(validateWifiExportPayload({ ...validPayload, exportedAt: 'now' })).toBeNull()
    expect(validateWifiExportPayload({ ...validPayload, generatedBy: 1 })).toBeNull()
    expect(validateWifiExportPayload({ ...validPayload, networks: 'x' })).toBeNull()
    expect(validateWifiExportPayload({ ...validPayload, samples: null })).toBeNull()
    expect(validateWifiExportPayload({ ...validPayload, connected: [] })).toBeNull()
  })

  it('rejects oversized networks and sample series', () => {
    expect(validateWifiExportPayload({ ...validPayload, networks: new Array(1001).fill({}) })).toBeNull()
    expect(validateWifiExportPayload({ ...validPayload, samples: { a: new Array(501).fill({ t: 1 }) } })).toBeNull()
    expect(validateWifiExportPayload({ ...validPayload, samples: { a: [{ t: 'x' }] } })).toBeNull()
  })
})
