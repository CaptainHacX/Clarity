...
import { collectLinuxFirewallStatus } from '../services/linux-firewall'
...
// ─── macOS / Linux ──────────────────────────────────────────────────────────
//
// socketfilterfw --listapps emits one line per app that has a socket filter
// rule, e.g.:
//
//   1234 : /Applications/Google Chrome.app    ( Allow incoming connections )
//   5678 : /System/Library/CoreServices/ARDAgent.app    ( Allow incoming connections )
//
// The numeric prefix is the app id; the parenthesised mode may be "Allow
// incoming connections", "Block incoming connections", or "No socketfilterfw
// rules apply". Paths may legally contain parentheses, so the mode is only
// parsed from the final parenthesised group at end-of-line.
const DARWIN_APP_LINE_RE = /^(\d+)\s*:\s*(.+?)\s*(?:\(([^)]*)\))?$/

export function parseDarwinAppLine(line: string): { id: string; path: string; allow: boolean } | null {
  const trimmed = line.trim()
  if (!trimmed) return null
  const m = trimmed.match(DARWIN_APP_LINE_RE)
  if (!m) return null
  const path = m[2].trim()
...
function isDarwinSystemPath(p: string): boolean {
  return p.startsWith('/System/') || p.startsWith('/usr/') || p.startsWith('/Library/Apple/')
}

const SOCKETFILTERFW = '/usr/libexec/ApplicationFirewall/socketfilterfw'

async function scanDarwinFirewall(onProgress?: (data: FirewallScanProgress) => void): Promise<FirewallScanResult> {
  onProgress?.({ phase: 'enumerating', current: 0, total: 0, currentRule: 'Checking macOS Application Firewall state...' })

  let globalEnabled = false
  try {
    const { stdout } = await execFileAsync(SOCKETFILTERFW, ['--getglobalstate'], { timeout: 10_000 })
    globalEnabled = /enabled/i.test(stdout)
  ...
  let listOutput = ''
  try {
    const { stdout } = await execFileAsync(SOCKETFILTERFW, ['--listapps'], { timeout: 30_000 })
    listOutput = stdout
  ...
  apps.forEach((app, i) => {
    ...
    if (i % 25 === 0) {
      onProgress?.({ phase: 'classifying', current: i + 1, total: apps.length, currentRule: program })
    }
  })
  ...
  return { rules, totalCount: rules.length, staleCount, unsignedCount, broadScopeCount }
}

const LINUX_TOOL_LABEL: Record<string, string> = {
  ufw: 'ufw',
  firewalld: 'firewalld',
  nftables: 'nftables',
  iptables: 'iptables',
...
async function scanLinuxFirewall(onProgress?: (data: FirewallScanProgress) => void): Promise<FirewallScanResult> {
  onProgress?.({ phase: 'enumerating', current: 0, total: 0, currentRule: 'Detecting active firewall...' })

  const status = await collectLinuxFirewallStatus()
  const toolLabel = LINUX_TOOL_LABEL[status.tool] ?? status.tool
  const rules: FirewallRule[] = []

  if (status.tool === 'none' || !status.active) {
    rules.push({
      name: 'firewall-inactive',
      displayName: 'No active firewall detected',
      description:
        'No front-end firewall (ufw or firewalld) is running and no nftables/iptables ' +
        'chain is filtering incoming traffic. Incoming connections on all ports are accepted.',
      group: '',
      profiles: ['Public'],
      protocol: 'Any',
      localPort: 'Any',
  ...
  } else if (status.allowedPorts.length === 0) {
    rules.push({
      name: 'firewall-default-deny',
      displayName: `${toolLabel} active — no inbound allows`,
      description: `The active firewall (${toolLabel}) reports no rules allowing inbound connections.`,
      group: '',
      profiles: ['Public'],
      protocol: 'Any',
      localPort: 'Any',
    ...
    status.allowedPorts.forEach((port, i) => {
      rules.push({
        name: `port-${port}`,
        displayName: `Inbound port ${port}`,
        description: `Port ${port} is allowed inbound by ${toolLabel}.`,
        group: '',
        profiles: ['Public'],
        protocol: 'Any',
      ...
      if (i % 25 === 0) {
        onProgress?.({ phase: 'classifying', current: i + 1, total: status.allowedPorts.length, currentRule: String(port) })
      }
    })
  }

  const broadScopeCount = rules.filter((r) => r.issues.includes('broad-scope')).length
  return { rules, totalCount: rules.length, staleCount: 0, unsignedCount: 0, broadScopeCount }
}

export async function scanFirewallRules(
  onProgress?: (data: FirewallScanProgress) => void
): Promise<FirewallScanResult> {
  if (process.platform === 'darwin') return scanDarwinFirewall(onProgress)
  if (process.platform === 'linux') return scanLinuxFirewall(onProgress)
...
export async function applyFirewallChanges(
  ...
  if (process.platform === 'darwin') return applyDarwinFirewallChanges(changes)
  if (process.platform === 'linux') return applyLinuxFirewallChanges(changes)
  if (process.platform !== 'win32') {
    return { succeeded: 0, failed: changes.length, errors: changes.map((c) => ({ name: c.name, displayName: c.name, reason: 'Firewall audit is not supported on this platform' })) }
  }
...
async function applyDarwinFirewallChanges(
  changes: { name: string; action: FirewallAction }[]
): Promise<FirewallApplyResult> {
  let succeeded = 0
  const errors: { name: string; displayName: string; reason: string }[] = []
  ...
  for (const c of changes) {
    ...
    // name is the absolute app path captured by the scanner.
  ...
  return { succeeded, failed: changes.length - succeeded, errors }
}

async function applyLinuxFirewallChanges(
  changes: { name: string; action: FirewallAction }[]
): Promise<FirewallApplyResult> {
  const status = await collectLinuxFirewallStatus()
  const ufwUsable = status.tool === 'ufw' && status.active
  let succeeded = 0
  const errors: { name: string; displayName: string; reason: string }[] = []
  ...
  for (const c of changes) {
    ...
    if (!m) {
      errors.push({ name: c.name, displayName: c.name, reason: 'Only per-port allow rules can be modified' })
      continue
    }
    if (!ufwUsable) {
      errors.push({ name: c.name, displayName: c.name, reason: 'Only ufw allow rules can be modified' })
      continue
    }
    try {
      ...
      errors.push({
        name: c.name,
        displayName: c.name,
        reason: 'Failed to disable rule — requires administrator privileges',
      })
    }
  }
