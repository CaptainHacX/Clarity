  ...
  try {
    const script = `
      Get-ScheduledTask | ForEach-Object {
        ...
        if ($hasLogon -and $task.TaskName -ne 'ClarityStartup' -and $task.TaskPath -notmatch '^\\\\Microsoft\\\\' -and $task.TaskPath -notmatch '^\\\\ASUS\\\\') {
...
export async function listStartupItems(): Promise<StartupItem[]> {
    ...
    // forever — surviving registry cleaning and even a Clarity reinstall, since
