...
export function registerSystemCleanerIpc(getWindow: WindowGetter): void {
  ipcMain.handle(IPC.SYSTEM_SCAN, () => runSystemScan(getWindow))
...
/**
 * Run a full system-junk scan. Shared by the renderer IPC handler and the
 * tray "Quick Scan" action so both report identical results. Progress is
 * streamed to the main window when one is open.
 */
export async function runSystemScan(getWindow: WindowGetter): Promise<ScanResult[]> {
