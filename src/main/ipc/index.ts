...
import { registerAlertMonitorIpc } from './alert-monitor.ipc'
import { registerNetworkSecurityIpc } from './network-security.ipc'
import { registerWifiIpc } from './wifi.ipc'
import { registerDevicesIpc } from './devices.ipc'
import { registerSecurityIpc } from './security.ipc'
import { registerHealthReportIpc } from './health-report.ipc'
...
import { registerPortManagerIpc } from './port-manager.ipc'
...
export function registerCleanerIpc(getWindow: WindowGetter): void {
  ...
  registerAlertMonitorIpc()
  registerNetworkSecurityIpc()
  registerWifiIpc(getWindow)
  registerDevicesIpc(getWindow)
  registerSecurityIpc()
  registerHealthReportIpc()
  ...
  ipcMain.handle(IPC.SETTINGS_SET, async (_event, settings) => {
    ...
    if (typeof validated.language === 'string') {
      await flushSettings()
      app.emit('clarity:language-changed')
    }
  ...
  ipcMain.handle(IPC.SETTINGS_SELECT_BACKUP_DIR, async () => {
    ...
    const opts: Electron.OpenDialogOptions = {
      title: 'Choose Clarity backup folder',
  ...
  ipcMain.handle(IPC.ELEVATION_RELAUNCH, () => {
    ...
    if (process.platform === 'win32') {
      ...
      parts.push(sq(exePath), '--no-sandbox', `--clarity-data-dir=${sq(userDataDir)}`)
  ...
  ipcMain.handle(IPC.RESTORE_POINT_CREATE, (_event, description: string) => {
    ...
    const sanitized = (description || 'Clarity pre-clean restore point')
  ...
  ipcMain.handle(IPC.DELETION_LOG_EXPORT, async (_event, query) => {
    ...
    const opts: Electron.SaveDialogOptions = {
      ...
      defaultPath: 'clarity-deleted-files.csv',
  ...
  // Threat monitor snapshot
  ipcMain.handle(IPC.THREAT_MONITOR_GET_SNAPSHOT, () => threatMonitor.getThreatSnapshot())
