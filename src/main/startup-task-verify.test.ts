...
const EXE = 'C:\\Program Files\\Clarity\\Clarity.exe'
const ARGS = '--startup'
...
describe('registeredTaskMatches', () => {
  ...
  it('accepts a path whose ampersand came back XML-escaped', () => {
    const exe = 'C:\\Tools\\R&D\\Clarity.exe'
    expect(registeredTaskMatches(withActions(execAction('C:\\Tools\\R&amp;D\\Clarity.exe')), exe, ARGS)).toBe(true)
  })
