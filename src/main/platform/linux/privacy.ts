...
const SYSCTL_CONF = '/etc/sysctl.d/99-clarity.conf'
...
const ACCESS_CONTROL_SETTINGS: PrivacySettingDef[] = [
  {
    ...
    async check() {
      try {
        ...
        const content = await readFile('/etc/security/limits.d/99-clarity.conf', 'utf8')
    ...
    async apply() {
      ...
      await writeFile('/etc/security/limits.d/99-clarity.conf', '* hard core 0\n', 'utf8')
    },
  },
  {
    id: 'ssh-root-login',
