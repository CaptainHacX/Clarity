...
const api = {
  ...
  // Settings
  settingsGet: (): Promise<ClaritySettings> => ipcRenderer.invoke(IPC.SETTINGS_GET),
  settingsSet: (settings: Partial<ClaritySettings>): Promise<void> =>
  ...
  onPerfHardwareHealth: (callback: (data: HardwareHealthSnapshot) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC.PERF_HARDWARE_HEALTH, handler) }
  },
  ...
  networkSecurityScan: (): Promise<NetworkSecurityStatus> => ipcRenderer.invoke(IPC.NETWORK_SECURITY_SCAN),
  networkSecurityRequestLocation: (): Promise<NetworkSecurityStatus> =>
    ipcRenderer.invoke(IPC.NETWORK_SECURITY_REQUEST_LOCATION),
  networkSecurityOpenLocationSettings: (): Promise<boolean> =>
    ipcRenderer.invoke(IPC.NETWORK_SECURITY_OPEN_LOCATION_SETTINGS),

  // Wi-Fi Scanner tool
  wifiScan: (detailed?: boolean): Promise<WifiScanSnapshot> =>
    ipcRenderer.invoke(IPC.WIFI_SCAN, detailed === true),
  wifiExport: (payload: WifiExportPayload): Promise<string | null> =>
    ipcRenderer.invoke(IPC.WIFI_EXPORT, payload),
  wifiLocationStatus: (): Promise<import('../shared/types').LocationAccessStatus> =>
    ipcRenderer.invoke(IPC.WIFI_LOCATION_STATUS),

  // Devices tool
  devicesScan: (): Promise<DevicesSnapshot> => ipcRenderer.invoke(IPC.DEVICES_SCAN),
  devicesInspectService: (
    request: import('../shared/types').ServiceInspectRequest,
  ): Promise<import('../shared/types').ServiceInspection | null> =>
    ipcRenderer.invoke(IPC.DEVICES_INSPECT_SERVICE, request),
  devicesOpenWebViewer: (request: {
    ip: string
    port: number
    scheme: 'http' | 'https'
    path?: string
  }): Promise<string | null> => ipcRenderer.invoke(IPC.DEVICES_OPEN_WEB_VIEWER, request),
  devicesMeasureLink: (request: LinkQualityRequest): Promise<LinkQuality | null> =>
    ipcRenderer.invoke(IPC.DEVICES_LINK_QUALITY, request),
  devicesProbe: (ip: string): Promise<DevicesProbeResult | null> =>
    ipcRenderer.invoke(IPC.DEVICES_PROBE_DEVICE, ip),
  devicesTagSet: (input: DeviceTagInput): Promise<DeviceTag | null> =>
    ipcRenderer.invoke(IPC.DEVICES_TAG_SET, input),
  devicesTagClear: (deviceId: string): Promise<boolean> =>
    ipcRenderer.invoke(IPC.DEVICES_TAG_CLEAR, deviceId),
  devicesHistoryGet: (): Promise<DeviceObservation[]> => ipcRenderer.invoke(IPC.DEVICES_HISTORY_GET),
  devicesHistoryClear: (): Promise<void> => ipcRenderer.invoke(IPC.DEVICES_HISTORY_CLEAR),
  devicesPortResultsSet: (deviceId: string, ports: import('../shared/types').DevicePortEntry[]): Promise<boolean> =>
    ipcRenderer.invoke(IPC.DEVICES_PORT_RESULTS_SET, { deviceId, ports }),

  // Network Security tool
  securityScanAll: (): Promise<SecuritySnapshot> => ipcRenderer.invoke(IPC.SECURITY_SCAN_ALL),
  securityScanDevice: (ip: string): Promise<DeviceSecurityResult | null> =>
    ipcRenderer.invoke(IPC.SECURITY_SCAN_DEVICE, ip),
  securityResultsGet: (): Promise<SecuritySnapshot> => ipcRenderer.invoke(IPC.SECURITY_RESULTS_GET),
  securityFullScanStart: (
    request: FullScanRequest,
  ): Promise<import('../shared/types').FullScanStartResult> =>
    ipcRenderer.invoke(IPC.SECURITY_FULL_SCAN_START, request),
  securityFullScanStatus: (ip: string): Promise<FullScanProgress | null> =>
    ipcRenderer.invoke(IPC.SECURITY_FULL_SCAN_STATUS, ip),
  securityFullScanCancel: (ip: string): Promise<boolean> =>
    ipcRenderer.invoke(IPC.SECURITY_FULL_SCAN_CANCEL, ip),
  securitySettingsGet: (): Promise<SecuritySettings> => ipcRenderer.invoke(IPC.SECURITY_SETTINGS_GET),
  securitySettingsSet: (patch: Partial<SecuritySettings>): Promise<SecuritySettings | null> =>
    ipcRenderer.invoke(IPC.SECURITY_SETTINGS_SET, patch),
  securityClearResults: (): Promise<boolean> => ipcRenderer.invoke(IPC.SECURITY_CLEAR_RESULTS),

  // System Health Report
  healthReportGenerate: (): Promise<SystemHealthReport> => ipcRenderer.invoke(IPC.HEALTH_REPORT_GENERATE),
  ...
  // Port Manager (Linux + macOS)
  portScan: (): Promise<PortScanResult> => ipcRenderer.invoke(IPC.PORT_SCAN),
  portKill: (pid: number): Promise<PortKillResult> =>
    ipcRenderer.invoke(IPC.PORT_KILL, pid),
  ...
  // Proactive Alerts
  alertsGetHistory: (): Promise<AlertEvent[]> => ipcRenderer.invoke(IPC.ALERT_GET_HISTORY),
  alertsClearHistory: (): Promise<boolean> => ipcRenderer.invoke(IPC.ALERT_CLEAR_HISTORY),
  onAlertEvent: (callback: (event: AlertEvent) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: AlertEvent) => callback(data)
    ipcRenderer.on(IPC.ALERT_EVENT, handler)
    return () => { ipcRenderer.removeListener(IPC.ALERT_EVENT, handler) }
  },
...
export type ClarityAPI = typeof api
