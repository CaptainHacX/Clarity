...
const CLEANER_TASKS: Record<string, {
  ...
  'cleaner:system': {
    label: 'System',
    scan: () => window.clarity.systemScan(),
    clean: (ids) => window.clarity.systemClean(ids)
  },
  'cleaner:browsers': {
    label: 'Browsers',
    scan: () => window.clarity.browserScan(),
    clean: (ids) => window.clarity.browserClean(ids)
  },
  'cleaner:apps': {
    label: 'Applications',
    scan: () => window.clarity.appScan(),
    clean: (ids) => window.clarity.appClean(ids)
  },
  'cleaner:gaming': {
    label: 'Gaming',
    scan: () => window.clarity.gamingScan(),
    clean: (ids) => window.clarity.gamingClean(ids)
  },
  'cleaner:recycleBin': {
    label: 'Recycle Bin',
    scan: () => window.clarity.recycleBinScan(),
    clean: () => window.clarity.recycleBinClean()
  },
  'cleaner:databases': {
    label: 'Databases',
    scan: () => window.clarity.databaseScan(),
    clean: (ids) => window.clarity.databaseClean(ids)
  }
}
  ...
  try {
    ...
    if (payload.autoApply && cleanerTasks.length > 0) {
      ...
      if (createRestorePoint) {
        try {
          await window.clarity.createRestorePoint(`Clarity scheduled clean — ${payload.scheduleName}`)
        } catch {
    ...
    if (payload.tasks.includes('registry')) {
      try {
        ...
        if (payload.autoApply && found > 0) {
          ...
          try {
            const result = await window.clarity.registryFix(ids)
    ...
    if (payload.tasks.includes('drivers')) {
      try {
        const result = await window.clarity.driverUpdateScan()
        const found = result.updates.length
        totalItems += found
        if (payload.autoApply && found > 0) {
          const ids = result.updates.map((u) => u.updateId)
          try {
            const installResult = await window.clarity.driverUpdateInstall(ids)
    ...
    if (payload.tasks.includes('software-update')) {
      try {
        ...
        if (payload.autoApply && found > 0) {
          ...
          try {
            const updateResult = await window.clarity.softwareUpdateRun(items)
    ...
    window.clarity.notifyScheduledScanComplete?.(totalSize, totalItems)
    window.clarity.scheduleRunComplete?.(payload.scheduleId, status)
    refreshSettings()
...
export function useScheduledScan(): void {
  ...
  useEffect(() => {
    if (!window.clarity?.onScheduleRunTrigger) return undefined
    ...
    const processQueue = async () => {
      while (queueRef.current.length > 0) {
        ...
        try {
          ...
          if (!idle) {
            window.clarity.scheduleRunComplete?.(next.scheduleId, 'failed')
    ...
    const unsubscribe = window.clarity.onScheduleRunTrigger((payload: ScheduleRunPayload) => {
