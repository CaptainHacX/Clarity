...
const defaultInfo: PlatformInfo = {
  platform: 'win32',
  features: { registry: true, debloater: true, drivers: true, restorePoint: true, bootTrace: true, gameMode: true, firewallAudit: true, contextMenu: true, portManager: true },
}
...
export function usePlatformLoader(): PlatformInfo {
  ...
  useEffect(() => {
    window.clarity?.platformInfo?.().then(setInfo).catch(() => {})
  }, [])
  return info
}
