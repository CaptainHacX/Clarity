...
import type { PerfSnapshot, HardwareHealthSnapshot } from '@shared/types'

function makeHealth(): HardwareHealthSnapshot {
  return {
    timestamp: Date.now(),
    cpuTemperature: 61,
...
describe('perf-store', () => {
  ...
  it('starts with empty state', () => {
    ...
    expect(state.hardwareHealth).toBeNull()
  })

  it('setHardwareHealth updates hardware health', () => {
    const health = makeHealth()
    usePerfStore.getState().setHardwareHealth(health)
    expect(usePerfStore.getState().hardwareHealth).toEqual(health)
  })
  ...
  it('reset preserves systemInfo but clears monitoring data', () => {
    ...
    usePerfStore.getState().setHardwareHealth(makeHealth())
    usePerfStore.getState().reset()
    const state = usePerfStore.getState()
