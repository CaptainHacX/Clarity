...
export function DiskAnalyzerPage() {
  ...
  useEffect(() => {
    if (drives.length === 0) {
      window.clarity?.diskDrives?.().then(store.setDrives).catch((err) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await window.clarity.diskAnalyze(selectedDrive)
  ...
  const handleFileTypeScan = async () => {
    ...
    try {
      const result = await window.clarity.diskFileTypes(selectedDrive)
