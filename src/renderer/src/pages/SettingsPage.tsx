...
import { Plus, X, FolderOpen, Sun, Moon, Monitor, RotateCcw, ShieldCheck } from 'lucide-react'
...
export function SettingsPage() {
  ...
  useEffect(() => { window.clarity?.settingsGet?.().then(setSettings).catch(() => {}) }, [])
  ...
  return (
    <div className="animate-fade-in max-w-2xl">
      ...
      <Section title={t('sectionAlerts')}>
        <Row label={t('alertsEnabledLabel')} desc={t('alertsEnabledDesc')}>
          <Toggle checked={settings.alerts.enabled} onChange={(v) => save({ alerts: { ...settings.alerts, enabled: v } })} />
        </Row>
        <Row label={t('alertsInAppLabel')} desc={t('alertsInAppDesc')}>
          <Toggle checked={settings.alerts.showInApp} onChange={(v) => save({ alerts: { ...settings.alerts, showInApp: v } })} />
        </Row>
        <Row label={t('alertsSystemLabel')} desc={t('alertsSystemDesc')}>
          <Toggle checked={settings.alerts.showSystem} onChange={(v) => save({ alerts: { ...settings.alerts, showSystem: v } })} />
        </Row>
        <Row label={t('alertsCpuUsageLabel')} desc={t('alertsCpuUsageDesc')}>
          <NumberSelect value={settings.alerts.cpuUsageThreshold} options={[70, 80, 85, 90, 95]}
            suffix="%"
            onChange={(v) => save({ alerts: { ...settings.alerts, cpuUsageThreshold: v } })} />
        </Row>
        <Row label={t('alertsCpuTempLabel')} desc={t('alertsCpuTempDesc')}>
          <NumberSelect value={settings.alerts.cpuTempThreshold} options={[80, 85, 90, 95, 100]}
            suffix="°C"
            onChange={(v) => save({ alerts: { ...settings.alerts, cpuTempThreshold: v } })} />
        </Row>
        <Row label={t('alertsMemoryLabel')} desc={t('alertsMemoryDesc')}>
          <NumberSelect value={settings.alerts.memoryThreshold} options={[80, 85, 90, 95]}
            suffix="%"
            onChange={(v) => save({ alerts: { ...settings.alerts, memoryThreshold: v } })} />
        </Row>
        <Row label={t('alertsDiskLabel')} desc={t('alertsDiskDesc')}>
          <NumberSelect value={settings.alerts.diskSpaceThresholdGb} options={[5, 10, 20, 50]}
            suffix="GB"
            onChange={(v) => save({ alerts: { ...settings.alerts, diskSpaceThresholdGb: v } })} />
        </Row>
        <Row label={t('alertsBatteryLabel')} desc={t('alertsBatteryDesc')}>
          <NumberSelect value={settings.alerts.batteryThreshold} options={[10, 15, 20, 25, 30]}
            suffix="%"
            onChange={(v) => save({ alerts: { ...settings.alerts, batteryThreshold: v } })} />
        </Row>
        <Row label={t('alertsCooldownLabel')} desc={t('alertsCooldownDesc')} last>
          <NumberSelect value={settings.alerts.cooldownMinutes} options={[5, 10, 30, 60, 120]}
            suffix={t('alertsCooldownUnit')}
            onChange={(v) => save({ alerts: { ...settings.alerts, cooldownMinutes: v } })} />
        </Row>
      </Section>
...
function SecuritySettingsSection() {
  ...
  const save = (patch: Partial<SecuritySettings>) => {
    void updateSettings(patch).then(() => toast.success(t('settingsSaved')))
  }
  ...
  const addCustomPort = () => {
    ...
    if (settings.customPorts.some((c) => c.port === port)) {
      toast.error(t('customInvalid'))
      return
    }
    save({ customPorts: [...settings.customPorts, { port, description: newDesc.trim() }] })
    setNewPort('')
    setNewDesc('')
  }
  ...
  return (
    <Section title={t('settingsTitle')}>
      <div className="flex items-center gap-2 pb-1">
        <ShieldCheck className="h-4 w-4" style={{ color: 'var(--accent)' }} strokeWidth={1.8} />
        <span className="text-[13px] font-medium text-zinc-300">{t('scheduleTitle')}</span>
      </div>
      <Row label={t('autoProbeLabel')} desc={t('autoProbeDesc')}>
        <Toggle
          checked={settings?.autoProbeEnabled ?? false}
          onChange={(v) => save({ autoProbeEnabled: v })}
        />
      </Row>
      <Row label={t('intervalHours')} desc={t('autoProbeDesc')}>
        <NumberSelect
          value={settings?.autoProbeIntervalHours ?? 6}
          options={[1, 3, 6, 12, 24, 168]}
          suffix={t('intervalHoursUnit', 'h')}
          onChange={(v) => save({ autoProbeIntervalHours: v })}
        />
      </Row>
      <Row label={t('inspectAutoLabel')} desc={t('inspectAutoDesc')} last>
        <Toggle
          checked={settings?.inspectAutomatically ?? true}
          onChange={(v) => save({ inspectAutomatically: v })}
        />
      </Row>
      <div className="mt-4 border-t pt-4" style={{ borderColor: 'var(--border-subtle)' }}>
        <p className="text-[13px] font-medium text-zinc-300">{t('customPortsTitle')}</p>
        <p className="mt-0.5 text-[12px]" style={{ color: 'var(--text-muted)' }}>{t('customPortsDesc')}</p>
        <div className="mt-3 space-y-2">
          {(settings?.customPorts ?? []).length === 0 && (
            <p className="text-[12px]" style={{ color: 'var(--text-faint)' }}>{t('customPortsEmpty', 'No custom ports yet.')}</p>
          )}
          {(settings?.customPorts ?? []).map((c) => (
            <div key={c.port} className="flex items-center justify-between rounded-xl px-4 py-2.5" style={{ background: 'var(--bg-subtle)' }}>
              <div className="flex items-center gap-2.5">
                <span className="font-mono text-[12px] font-semibold text-zinc-300">{c.port}</span>
                {c.description && (
                  <span className="text-[12px]" style={{ color: 'var(--text-muted)' }}>{c.description}</span>
                )}
              </div>
              <button
                onClick={() => save({ customPorts: (settings?.customPorts ?? []).filter((x) => x.port !== c.port) })}
          ...
          <div className="flex items-center gap-2.5">
            <input
              ...
              className="w-28 rounded-xl px-4 py-2.5 font-mono text-[13px] text-zinc-300 outline-none placeholder:text-zinc-700"
              style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-medium)' }}
            />
            <input
              type="text"
...
function BackupFolderRow({
  ...
  const displayPath = isCustom ? path : t('backupFolderDefaultLabel', 'Default (Documents/Clarity Backups)')
  return (
    <div className="space-y-3">
      <div>
        <p className="text-[13px] font-medium text-zinc-300">{t('backupFolderLabel', 'Backup folder')}</p>
        <p className="mt-0.5 text-[12px]" style={{ color: 'var(--text-muted)' }}>
          {t('backupFolderDesc', 'Where Clarity writes registry and shell-extension backups before making changes. Existing backups stay in their previous location when you switch folders.')}
        </p>
      </div>
      <div className="flex items-center gap-2.5">
        <div
...
function NumberSelect({ value, options, suffix, onChange }: { value: number; options: number[]; suffix?: string; onChange: (v: number) => void }) {
  return (
    <select
      value={value}
