...
export function LargeFileFinderPage() {
  ...
  useEffect(() => {
    if (!window.clarity?.onLargeFilesProgress) return
    return window.clarity.onLargeFilesProgress((data) => {
  ...
  const handleSelectDir = async () => {
    const dir = await window.clarity?.largeFilesSelectDir?.()
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await window.clarity?.largeFilesScan?.({
  ...
  const handleCancel = async () => {
    await window.clarity?.largeFilesCancel?.()
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await window.clarity?.largeFilesDelete?.(paths, store.deleteMode)
      if (result) {
                ...
                {store.result.files.map((file, idx) => (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        window.clarity?.largeFilesOpenLocation?.(file.path)
                      }}
