...
export function DriverManagerPage({ embedded }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    const cleanupDriver = window.clarity.onDriverProgress((data: DriverScanProgress) => {
      useDriverStore.getState().setScanProgress(data)
    })
    const cleanupUpdate = window.clarity.onDriverUpdateProgress((data: DriverUpdateProgress) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    const [staleResult, updateResult] = await Promise.allSettled([
      window.clarity.driverScan(),
      window.clarity.driverUpdateScan()
    ])
  ...
  const handleApply = useCallback(async () => {
    ...
    if (selectedUpdates.length > 0) {
      ...
      try {
        const result = await window.clarity.driverUpdateInstall(ids)
    ...
    if (selectedStale.length > 0) {
      ...
      try {
        const result = await window.clarity.driverClean(names)
