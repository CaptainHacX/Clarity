...
function RegistryPageContent() {
  ...
  useEffect(() => {
    const cleanup = window.clarity.onRegistryFixProgress((data) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    store.setError(null)
    try {
      const results = await window.clarity.registryScan()
  ...
  const handleScanCancel = useCallback(async () => {
    try {
      await window.clarity.registryScanCancel()
    } catch { /* ignore */ }
    useRegistryStore.getState().setScanning(false)
  }, [])
  ...
  const handleFixCancel = useCallback(async () => {
    try {
      await window.clarity.registryFixCancel()
    } catch { /* ignore */ }
  ...
  const handleFix = useCallback(async () => {
    ...
    try {
      const result = await window.clarity.registryFix(selectedIds)
