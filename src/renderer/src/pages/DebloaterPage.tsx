...
export function DebloaterPage({ embedded }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    const cleanup = window.clarity.onDebloaterRemoveProgress((data) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      const results = await window.clarity.debloaterScan()
  ...
  const handleRemove = useCallback(async () => {
    ...
    try {
      const result = await window.clarity.debloaterRemove(selectedPkgs)
