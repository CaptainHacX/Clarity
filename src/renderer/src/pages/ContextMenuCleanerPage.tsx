...
const WIN11_NOTICE_KEY = 'clarity.contextMenu.win11Notice.dismissed'
...
function ContextMenuCleanerPageContent() {
  ...
  useEffect(() => {
    const cleanup = window.clarity.onContextMenuApplyProgress((data) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    store.setError(null)
    try {
      const result = await window.clarity.contextMenuScan()
  ...
  const handleScanCancel = useCallback(async () => {
    try { await window.clarity.contextMenuScanCancel() } catch { /* ignore */ }
    useContextMenuStore.getState().setScanning(false)
  }, [])
  ...
  const handleApply = useCallback(async (action: ContextMenuAction, requests?: ContextMenuApplyRequest[]) => {
    ...
    try {
      const result = await window.clarity.contextMenuApply(reqs)
    ...
    <div className="animate-fade-in">
      ...
      {requiresAdminInSelection && selectedCount > 0 && !applying && (
        <div className="mb-3 flex items-center gap-2.5 rounded-xl px-4 py-2.5 text-[12px]"
          ...
          <button
            onClick={() => window.clarity.elevationRelaunch?.().catch(() => {})}
