...
export function GameModePage() {
  ...
  const handleActivate = useCallback(async () => {
    ...
    progressCleanupRef.current = window.clarity?.onGameModeProgress?.((data) => {
      useGameModeStore.getState().setProgress(data)
    }) ?? null

    try {
      const result = await window.clarity.gameModeActivate(config)
  ...
  const handleDeactivate = useCallback(async () => {
    ...
    try {
      const result = await window.clarity.gameModeDeactivate()
  ...
  const handleDiscardPending = useCallback(async () => {
    try {
      const result = await window.clarity.gameModeDiscardPending()
      if (!result.discarded) {
