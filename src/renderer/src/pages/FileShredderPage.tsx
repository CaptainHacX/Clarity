...
export function FileShredderPage() {
  ...
  useEffect(() => {
    if (!window.clarity?.onShredderProgress) return
    return window.clarity.onShredderProgress((data) => {
  ...
  const handleAddFiles = async () => {
    const entries = await window.clarity?.shredderSelectFiles?.()
    if (entries?.length) store.addEntries(entries)
  }

  const handleAddFolders = async () => {
    const entries = await window.clarity?.shredderSelectFolders?.()
    if (entries?.length) store.addEntries(entries)
  }
  ...
  const handleShred = async () => {
    ...
    try {
      ...
      const result = await window.clarity?.shredderShred?.(paths)
      if (result) {
  ...
  const handleCancel = async () => {
    await window.clarity?.shredderCancel?.()
  }
          ...
          >
            {store.entries.map((entry, idx) => (
              ...
              >
                ...
                <button
                  onClick={() => window.clarity?.shredderOpenLocation?.(entry.path)}
