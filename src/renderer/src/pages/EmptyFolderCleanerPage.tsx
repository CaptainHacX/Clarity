...
export function EmptyFolderCleanerPage() {
  ...
  useEffect(() => {
    if (!window.clarity?.onEmptyFoldersProgress) return
    return window.clarity.onEmptyFoldersProgress((data) => {
  ...
  const handleSelectDir = async () => {
    const dir = await window.clarity?.emptyFoldersSelectDir?.()
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await window.clarity?.emptyFoldersScan?.({
  ...
  const handleCancel = async () => {
    await window.clarity?.emptyFoldersCancel?.()
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await window.clarity?.emptyFoldersDelete?.(paths, store.deleteMode)
      if (result) {
                ...
                {store.result.folders.map((folder, idx) => (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        window.clarity?.emptyFoldersOpenLocation?.(folder.path)
                      }}
