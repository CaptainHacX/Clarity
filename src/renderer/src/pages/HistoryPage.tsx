...
function DeletedFilesSection({ from, to }: { from: string; to: string }) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    // Only local-origin deletions belong to this history entry.
    window.clarity.deletionLogQuery({ from, to, origin: 'local', offset: 0, limit: DELETED_FILES_PAGE })
      .then((page) => {
        if (cancelled) return
  ...
  const loadMore = async () => {
    setLoadingMore(true)
    try {
      const page = await window.clarity.deletionLogQuery({
  ...
  const handleExport = async () => {
    setExporting(true)
    try {
      const savedTo = await window.clarity.deletionLogExport({ from, to, origin: 'local' })
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        {total > 0 && (
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <IconButton icon={FolderOpen} label={t('detail.deletedFiles.openLog')} onClick={() => window.clarity.deletionLogReveal()} />
          </div>
        )}
      </div>
