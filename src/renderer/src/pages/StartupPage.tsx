...
import { Zap, Shield, RefreshCw, Clock, Activity, TrendingDown, ChevronDown, ChevronUp, BarChart3, Trash2, PackageX } from 'lucide-react'
...
export function StartupPage() {
  ...
  const loadBootTrace = useCallback(async () => {
    ...
    try {
      const trace = await window.clarity.startupBootTrace()
  ...
  const handleToggle = async (item: StartupItem, enabled: boolean) => {
    ...
    try {
      const success = await window.clarity.startupToggle(item.name, item.location, item.command, item.source, enabled)
      if (!success) {
  ...
  const handleDelete = async (item: StartupItem) => {
    const startTime = Date.now()
    try {
      const success = await window.clarity.startupDelete(item.name, item.source === 'startup-folder' ? item.command : item.location, item.source)
      if (success) {
