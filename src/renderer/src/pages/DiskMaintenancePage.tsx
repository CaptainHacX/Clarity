...
export function DiskMaintenancePage() {
  ...
  useEffect(() => {
    if (!window.clarity?.onDiskTrimProgress) return
    return window.clarity.onDiskTrimProgress((data) => store.setProgress(data))
  }, [])
  ...
  async function refresh(): Promise<void> {
    ...
    store.setError(null)
    try {
      const list = await window.clarity.diskTrimList()
  ...
  async function handleRun(): Promise<void> {
    ...
    try {
      const results = await window.clarity.diskTrimRun(selectableSelected)
