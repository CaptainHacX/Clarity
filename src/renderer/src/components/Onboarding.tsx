...
export function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const applyAndFinish = async () => {
    ...
    try {
      ...
      await window.clarity?.settingsSet?.(settingsPayload)
      await window.clarity?.applyStartup?.(settings.runAtStartup).catch(() => {})
      window.clarity?.applyTray?.(settings.minimizeToTray)
    } catch {
      // Best-effort
    }
  }
...
function LanguageStep({ onNext }: { onNext: () => void }) {
  ...
  const handleSelect = (code: string) => {
    ...
    window.clarity?.settingsSet?.({ language: code }).catch(() => {})
  }
...
function WelcomeStep({ onBack, onNext }: { onBack: () => void; onNext: () => void }) {
  ...
  return (
    <StepWrapper>
      <div className="flex flex-col items-center text-center">
        <img src={logoSrc} alt="Clarity" className="mb-5 h-20 w-20 rounded-2xl" />
