...
if (expectedEntries.length > 0 && !wroteEntries) {
  ...
  console.error('\nThis usually means the changelog preset failed to load, or loaded in a')
  console.error('shape the generator could not read. Both packages must be local')
  console.error('devDependencies, and their majors must match — the CLI resolves the preset')
  console.error("from this project's node_modules, and conventional-changelog@8 only reads")
  console.error('the preset shape that conventional-changelog-angular@9 exports:\n')
  console.error('  npm ls conventional-changelog conventional-changelog-angular\n')
  console.error('  expected: conventional-changelog@8.x  conventional-changelog-angular@9.x\n')
